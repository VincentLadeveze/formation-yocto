...
==============
    ...
    -------------------------
Quatre cœurs, 8 Go de RAM et une centaine de Go de disque
...
# Image pour BeagleBone Black
    ...
    # Branchement du câble Sertronics usb-to-ttl converter
...
Chapter 2       Début de la compilation avec 'core-image-base'
=========
...
---        ==> Cloner n'est aps forcément utile ?...
...
# Ajout d'un patch dans une recette (patch d'un fichier présent dans une recette : pas de compilation mais build avec 'bitbake')
    RAS
# Patch sur un fichier source de package (patch s’appliquant sur un fichier source d’un package téléchargé et compilé par une recette)
...
Voir log dans tmp/work/qemuarm-poky-linux-gnueabi/tutorial/1.0-r0/temp/log.do_rootfs.31401
...
Chapter 4         Ajuster l'image Yocto pour une cible personnalisée
=========
  ...
  # Configuration initiale du réseau:  se fait en Ethernet testé sur ma RPI4
    ### TROUBLE:    Ethernet testé (clignotement côté FreeBox) avec mon PC mais PAS AVEC MA RPI4...

    ### WORKAROUND:         Utilisation du Wifi (le Wifi est testé après install de Raspberry PI OS avec SSID/Mdp de la Freebox)


    ###########################################################################################################

        ##
        ## WPA Supplicant Nasty Wifi configuration  -     Static IP Address
        ##
...
NB: 
    ...
    - adresse IP statique.
        ...
        ##
        ## Network Interfaces Wifi configuration  -   Dynamic IP Address
        ##
            ...
            # YOCTO
             ...
             ## 1/ wpa_supplicant
              ...
              # Codage du mot de passe wifi avec 'wpa_supplicant'  (à tester manuellement d'abord)
                $ wpa_passphrase Freebox-9B920B jocasse3-adsignandi\!-admittebat-cuppes? > home-wifi.conf
            ...
            ## 2/ Interfaces réseau
              ...
              # Ajout automatique de l'interface Wifi
              ## METHODE 1
...
NB:     Meilleure solution que le script utilisant 'wpa_supplicant' mais pas encore optimum pour la sécurité.
    ...
    ###########################################################################################################
    ##############################   Repise du cour de la formation ###########################################
    ###########################################################################################################


  # Accès internet via DNS
...
CHAPTER 5           => idée pour prolonger la formation
=========
